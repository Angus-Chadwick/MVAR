function [SI_model_rel_full, SI_model_irrel_full, SI_model_rel, SI_model_irrel, Model_rel, Model_irrel, Model_rel_full, Model_irrel_full, CELLLAB_TOT] = model_SI_switching(changeI, changeW)
...
[SI, poolvar] = selectivity_analytic_switching(manipulation, var_method, W1, W2, xrel, xirrel, rmat_rel, rmat_irrel, ResidualV_rel_TOT, ResidualV_irrel_TOT, ResidualA_rel_TOT, ResidualA_irrel_TOT,vmatrel0, vmatirrel0, Trialsrel_all, Trialsirrel_all, tsims, rngtot, CELLLAB_ALL,RXS);

SI_model_rel_full = SI.SIinputs(:,1) + SI.SIinitstate(:,1) + SI.SIvel(:,1) + SI.SIres(:,1);
SI_model_irrel_full = SI.SIinputs(:,2) + SI.SIinitstate(:,2) + SI.SIvel(:,2) + SI.SIres(:,2);
...
LrelA = size(ResidualA_rel_TOT{rix}, 3); LirrelA = size(ResidualA_irrel_TOT{rix}, 3);
for L = 1:(LrelA)
ResidualA_TOT{rix}(:,:,L) = ResidualA_rel_TOT{rix}(:,:,L);
end
for L = 1:(LirrelA)
ResidualA_TOT{rix}(:,:,LrelA + L) = ResidualA_irrel_TOT{rix}(:,:,L);
end
...
if strmatch(shufflemethod, 'full')
   ...
   for i=1:size(ResidualV_TOT{rix},1) 
      ...
      % if ismember(i, dum)      
            ...
            ResidualA_TOT_shuff{rix}(i,:,:) = ResidualA_TOT{rix}(i,:,randperm(LrelA + LirrelA));
...
elseif strmatch(shufflemethod, 'trials')
    ...
    ResidualA_TOT_shuff{rix} = ResidualA_TOT{rix}(:,:,randperm(LrelA + LirrelA));
    ResidualV_TOT_shuff{rix} = ResidualV_TOT{rix}(:,:,randperm(LrelV + LirrelV));

end
    
ResidualA_rel_TOT_shuff{rix} = ResidualA_TOT_shuff{rix}(:,:, 1:size(ResidualA_rel_TOT{rix}, 3));
ResidualA_irrel_TOT_shuff{rix} = ResidualA_TOT_shuff{rix}(:,:, (size(ResidualA_rel_TOT{rix}, 3)+1):end);
...
[SI, poolvar] = selectivity_analytic_relirrelshuff(xrel, xirrel, rmat_rel, rmat_irrel, ResidualV_rel_TOT_shuff, ResidualV_irrel_TOT_shuff, ResidualA_rel_TOT_shuff, ResidualA_irrel_TOT_shuff,vmatrel0, vmatirrel0, Trialsrel_all, Trialsirrel_all, tsims, rngtot, CELLLAB_ALL,RXS);

SI_model_rel{S} = SI.SIinputs(:,1) + SI.SIinitstate(:,1) + SI.SIvel(:,1) + SI.SIres(:,1);
SI_model_irrel{S} = SI.SIinputs(:,2) + SI.SIinitstate(:,2) + SI.SIvel(:,2) + SI.SIres(:,2);
% SI_model_rel_deltares = SI_model_rel_full - SI.SIdeltares(:,1)  ;  % amount generated without single-cell residuals
% SI_model_irrel_deltares = SI_model_irrel_full - SI.SIdeltares(:,2) ; % amount generated without single-cell residuals

for j=1:4
    Model_rel{S}(j) = mean(abs(SI_model_rel{S}(CELLLAB_TOT == CLS(j))));
    Model_irrel{S}(j) = mean(abs(SI_model_irrel{S}(CELLLAB_TOT == CLS(j))));
...
%     Model_rel_deltares(j) = mean(abs(SI_model_rel_deltares(CELLLAB_TOT == CLS(j))));
%     Model_irrel_deltares(j) = mean(abs(SI_model_irrel_deltares(CELLLAB_TOT == CLS(j))));
end
...
%     for W2 = [1,3,4,5]
% 
%         [SI{W1,W2}, poolvar{W1,W2}] = selectivity_analytic(manipulation, var_method, W1, W2, xrel, xirrel, rmat_rel, rmat_irrel, ResidualV_rel_TOT, ResidualV_irrel_TOT, ResidualA_rel_TOT, ResidualA_irrel_TOT,vmatrel0, vmatirrel0, Trialsrel_all, Trialsirrel_all, tsims, rngtot, CELLLAB_ALL);
%        
%     end
% end
