function [xrel, xirrel, Measurement_rel, Measurement_irrel, prediction_rel, prediction_irrel] = fitLDS_Switching(fixvars, changeW, changeI, drmat0rel, drmat0irrel, rmat0rel, rmat0irrel, rngtot, Ntrialsrel, Ntrialsirrel, vmatSTM_rel0, vmatSTM_irrel0, CELLLAB_ALL)
...
%                        repmat(zeros(length(rngtot)), [1, Ntrialsirrel{1}, 1, 1]), repmat(eye(length(rngtot)), [1, Ntrialsirrel{2}, 1, 1])];
    ...
    vrel =  vertcat(vmatSTM_rel0{1:Ncond}).';              
    virrel =  vertcat(vmatSTM_irrel0{1:Ncond}).';                                  
   
    Lambdatot_fixI = [rreltot, zeros(size(rirreltot)); zeros(size(rreltot)), rirreltot; stimblocksrel, stimblocksirrel; vrel, virrel];  % only allow weights to change                                     
    Lambdatot_fixW = [rreltot, rirreltot; stimblocksrel, zeros(size(stimblocksirrel)); zeros(size(stimblocksrel)), stimblocksirrel; vrel, virrel];  % only allow weights to change                                     
...
elseif strmatch(fixvars, 'Mixed') % allow only some inputs to change
    ...
    for j=1:4  % for predictions of cell type j
        ...
        Lambdatot_partialW_ChangeI = [rreltot, rirrelWfixed;  zeros(size(rreltot)), rirrelWchange; stimblocksrel, zeros(size(stimblocksirrel)); zeros(size(stimblocksrel)), stimblocksirrel; vrel, virrel];  % only allow weights to change                                     
        Lambdatot_partialW_fixI    = [rreltot, rirrelWfixed;  zeros(size(rreltot)), rirrelWchange; stimblocksrel, stimblocksirrel; vrel, virrel];
            ...
            if ~changeI(j)
             ...
             elseif changeI(j) 
                        ...
                        xtot{j}(:, (find(~ismember(CELLLAB_ALL, changeWlin))) + length(CELLLAB_ALL)) = xtot{j}(:, ~ismember(CELLLAB_ALL, changeWlin));
                        xrel(CELLLAB_ALL == CLS(j),:)   = xtot{j}(:, [1:length(CELLLAB_ALL),  (2*length(CELLLAB_ALL) + 1):(2*length(CELLLAB_ALL) + 17 * Ncond), end]);
