function [xpre, xpost, Measurement_pre, Measurement_post, Prediction_pre, Prediction_post] = fitLDS_weighted(fixvars, changeW, changeI, drmat0pre, drmat0post, rmat0pre, rmat0post, rngtot, Ntrialspre, Ntrialspost, vmatSTM_pre0, vmatSTM_post0, CELLLAB_ALL, D)
...
elseif strmatch(fixvars, 'Mixed') % allow only some inputs to change
    ...
    xpre(:, (length(CELLLAB_ALL) + 1):end) = inv(D) * xpre(:, (length(CELLLAB_ALL) + 1):end);
    xpost(:, (length(CELLLAB_ALL) + 1):end) = inv(D) * xpost(:, (length(CELLLAB_ALL) + 1):end);
